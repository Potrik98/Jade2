...
namespace hashkey {
    ...
    void init() {
        int piece_conv[13] = { -1, 1, 3, 5, 7, 9, 11, 0, 2, 4, 6, 8, 10 };

        for (int i = 1; i < 13; i++) {
            for (int j = 0; j < 64; j++) {
                int piece = piece_conv[i];
                int index = 64 * piece + j;
                keyPiece[i][SQ120(j)] = KEYS[index];
            }
        }
        for (int file = FILE_A; file <= FILE_H; file++) {
            int rank = RANK_6;
            keyPiece[0][FR2SQ(file, rank)] = KEYS[772 + file];
            rank = RANK_3;
            keyPiece[0][FR2SQ(file, rank)] = KEYS[772 + file];
        }
        keySide = KEYS[780];
        for (int i = 0; i < 16; i++) {
            keyCastle[i] = 0ULL;
            if (i & WKCA) keyCastle[i] ^= KEYS[768];
            if (i & WQCA) keyCastle[i] ^= KEYS[769];
            if (i & BKCA) keyCastle[i] ^= KEYS[770];
            if (i & BQCA) keyCastle[i] ^= KEYS[771];
        }
    }
}
...
U64 generatePosKey(const Board* board) {
    ...
    for (sq = 0; sq < 120; ++sq) {
        ...
        if (piece != NO_SQ && piece != EMPTY && piece != OFFBOARD) {
            finalKey ^= hashkey::keyPiece[piece][sq];
        }
    }

    if (board->sideToMove == WHITE) {
        finalKey ^= hashkey::keySide;
    }
